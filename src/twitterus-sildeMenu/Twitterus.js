...
function Twitterus(mount) {
    ...
    this.animation = new Position(this);
    this.animation2 = new MountPoint(this);
...
Twitterus.prototype.constructor = Twitterus;
...
Twitterus.prototype.onReceive = function onReceive (event, payload) {
    ...
    if (event === 'click') {
        ...
        this.emit('changeSection', {
            ...
            to: to || data.sections[0].id
        });

        // set the current section
        this.currentSection = to;
    }else if (event === 'initSection') {
      this.emit('changeSection', {from: null, to: data.sections[0].id});
    }else if(event === 'headerClick'){

        if(this.openStatus){
            //this.setPosition(0, 0, 0);
            this.animation.set(0, 0, 0, {
              duration: 0
            });
            this.animation2.set(0, 0, 0, {
              duration: 0
            });
